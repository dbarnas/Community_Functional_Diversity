...
# 
# # Alternative: Max (specifically for del15N and N_percent)
# Full_data <- ReducedChemData %>%
#   group_by(Location, CowTagID) %>%
#   # get parameter ranges across sampling periods by site and plate
#   summarise_at(vars(Salinity:Phosphate_umolL), .funs = max, na.rm=T) %>% # returns two rows containing max and min value per CowTagID
#   ungroup() %>% 
#   left_join(turb, by = 'CowTagID') %>%  # join with turb
#   # only keep Plate and Seep data
#   anti_join(removeSite) # ignores springs, pits, and offshore samples
...
Full_data %>% 
  select(Location, CowTagID, Silicate_umolL, del15N, N_percent) %>% 
...
%N ~ Silicate
```{r}
# quick function to get linear model of x parameter
# linearR2 <- function(x, y = Silicate_umolL) {
#   model <- lm(x ~ y)
#   val <- summary(model)$r.squared
#   return(val)
# }

# V_R2 <- Full_data %>% 
#   filter(Location == "Varari") %>% 
#   mutate(R2_Salinity = linearR2(Salinity))
#          R2_N_percent = linearR2(N_percent),
#          R2_del15N = linearR2(del15N),
#          R2_pH = linearR2(pH))

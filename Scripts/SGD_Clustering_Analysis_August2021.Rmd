...
Cluster_metadata <- V_Cluster_data %>% 
  select(Location, CowTagID, lat, lon, V_cluster)
write_csv(Cluster_metadata, here("Data","Surveys","Cluster_metadata.csv"))

```  

Check if clusters are significantly distinct
Incomplete script portion. Does not eval
```{r, eval = F}
# make cluster ID a factor
V_Cluster_data$V_cluster <- as.factor(V_Cluster_data$V_cluster)

# clustermodel <- lm(data = V_Cluster_data, V_cluster ~ Salinity * TA * pH * Phosphate_umolL * Silicate_umolL * NN_umolL * Ammonia_umolL * M_C * HIX * VisibleHumidic_Like * Tryptophan_Like * Tyrosine_Like * del15N * C_N * N_percent)

clustermodel <- lm(data = V_Cluster_data, V_cluster ~ Silicate_umolL)
...
library(emmeans)
graphdata<-as.data.frame(emmeans(clustermodel, ~ Silicate_umolL))
graphdata
...
### Save clusters in csv
Cluster_metadata_turb <- V_Cluster_data_turb %>% 
  select(Location, CowTagID, lat, lon, V_cluster_turb)
write_csv(Cluster_metadata_turb, here("Data","Surveys","Cluster_turb_metadata.csv"))
...
## create palette
V_cluster_palette_turb <- c("#fad02c", "white", "brown3") #3 is dark yellow #2 is baby blue #1 is red 

### Isolate seep data for point on map
VSeepPt_turb <- V_Cluster_data_turb %>% filter(CowTagID == "VSEEP") # isolate Varari seep

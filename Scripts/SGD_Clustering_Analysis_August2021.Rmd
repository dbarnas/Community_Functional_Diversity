...
## Turbinaria nitrogen relationships with silicate
Full_data %>% 
...
## Turbinaria nitrogen relationships with salinity
Full_data %>% 
  pivot_longer(cols = c(del15N:N_percent), names_to = "Parameters", values_to = "Values") %>% 
  filter(CowTagID != "VSEEP" & CowTagID != "CSEEP") %>% 
  ...
  geom_smooth(method = "lm") +
  facet_wrap(~Parameters, scales = "free") +
  labs(title = "T. ornata Nitrogen ~ Salinity",
       x = "Range of Salinity (psu)",
       y = "Value Ranges") +
  theme_bw() +
  theme(strip.background = element_rect(fill = "white"))
```

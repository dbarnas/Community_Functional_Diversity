...
maya <- species %>% 
  ...
  filter(CowTagID == "Reef_Ambient_2" | CowTagID == "Reef_Ambient_1" | 
           ...
           CowTagID ==  "VSEEP" | CowTagID == "V13") %>% # also removing seep and 13 for now
  distinct()
...
chem <- chem %>% 
  ...
  left_join(envi) %>% 
  select(-c(Location, lat, lon, Dist_CT_cm, Depth_cm, Tidal_diff))
...
full_comp <- species %>% 
  ...
  filter(CowTagID != "VSEEP", CowTagID != "V13") # remove seep and V13

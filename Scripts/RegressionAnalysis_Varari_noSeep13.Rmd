...
respval4 <- anova(lm(data = res_data, res ~ dist_to_seep_m))[5]$'Pr(>F)'[1]
resrval4 <- summary((lm(data = res_data, res ~ dist_to_seep_m)))$r.squared
...
res_data %>%
  ...
  geom_smooth(method = "lm", color = pretty_palette[5]) +
  theme_bw() + theme(panel.grid = element_blank()) +
  labs(x = "Distance to seep (m)",
       y = "Residuals (Richness ~ Rugosity)",
       caption = paste("p-value:",round(respval4,3),"\n r-squared:", round(resrval4, 2)))
...
# running nMDS
calcanMDS <- mynMDS_group(mdsord = calcOrd) # same as above
calcnMDS_ct_means <- mynMDS_CowTag(mdsord = calcOrd, param_df = res_data_means, 
                              param_select = c('CowTagID', 'dist_to_seep_m', 'VisibleHumidic_Like', 'Ammonia_umolL', 'del15N'))

# plotting
calcnMDS_meansPlot <- plotmynMDS(nMDSgroup = calcanMDS, nMDScowtag = calcnMDS_ct_means, param_fill = dist_to_seep_m) + labs(title = "Mean Values")
...
# plotting
speciesnMDSPlot <- plotmynMDS(nMDSgroup = speciesnMDS, nMDScowtag = speciesnMDS_ct, param_fill = dist_to_seep_m) + labs(title = "CV Values")

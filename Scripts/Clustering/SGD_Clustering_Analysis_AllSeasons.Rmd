...
Full_data_season<-read_csv(here("Data","Biogeochem","Nutrient_Processed_CV_seasons.csv"))
Full_data<-read_csv(here("Data","Biogeochem","Nutrient_Processed_CV.csv"))
head(Full_data)
```
...
arrange(as.data.frame(res_V@criteria@discrim), 'res_V@criteria@discrim')
Cdes<-plot(x = res_C, y = 1:ncol(x_C))
arrange(as.data.frame(res_C@criteria@discrim), 'res_C@criteria@discrim')


ggsave(here("Output","SGDZones","Varari",paste0("DiscPower_all_Varari.png")),Vdes, height = 10, width = 10)
ggsave(here("Output","SGDZones","Cabral",paste0("DiscPower_all_Cabral.png")),Cdes, height = 10, width = 10)
...
V_meta <- V_Cluster_data %>% rename(cluster = V_cluster) %>% mutate_at(vars(Salinity:N_percent),.funs = as.numeric) %>% 
  mutate(cluster = if_else(cluster == 3, "High", if_else(cluster == 2, "Low", "Mid")))
...
write_csv(mymeta, here("Data", "Biogeochem", "Cluster_metadata_FullSeasons.csv"))
```  
...
VmapSites <- ggmap(VarariBaseMap) +
  ...
  geom_label_repel(data = Full_data,
             aes(x = lon, y = lat,
                 label = CowTagID),
             size = 2) 
...
ggsave(here("Output","SGDZones","Varari",paste0("Varari_Cluster_Map_FullSeasons.png")),cluster_varari_map,height = 10, width = 10)
...
ggsave(here("Output","SGDZones","Cabral",paste0("Cabral_Cluster_Map_FullSeasons.png")),cluster_cabral_map,height = 10, width = 10)
...
ggsave(here("Output","SGDZones","Varari",paste0("Boxplots_Varari_FullSeasons.png")), Vbox, height = 20, width = 8)
```
...
ggsave(here("Output","SGDZones","Cabral",paste0("Boxplots_Cabral_FullSeasons.png")), Cbox, height = 20, width = 8)
```
...
p2_V<-PC_loadings_V %>%
  ...
  coord_cartesian(xlim = c(-6, 7), ylim = c(-7, 4)) +
   theme_bw()+
...
VarariPCA<-p1_V+p2_V+ 
  patchwork::plot_annotation("Varari Clusters", 
                             theme = theme(plot.title = element_text(size = rel(1.5), 
                                                                     ...
                                                                     margin = margin(t = 10, b = 20,
                                                                                     unit = "pt"))))
ggsave(plot = VarariPCA, filename = here("Output","SGDZones","Varari","VarariPCA_FullSeasons.png"), width = 14, height = 6)
...
p2_C<-PC_loadings_C %>%
  ...
  coord_cartesian(xlim = c(-7, 4), ylim = c(-6, 7)) +
   theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
ggsave(plot = CabralPCA, filename = here("Output","SGDZones","Cabral","CabralPCA_FullSeasons.png"), width = 14, height = 6)

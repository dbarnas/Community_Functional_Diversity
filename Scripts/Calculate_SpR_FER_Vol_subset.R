...
# removing abiotics then getting species above 5% of total biotic cover
...
myspecies <- myspecies %>%
  ...
  # remove species with less than 10% biotic cover
  filter(cover > 5) %>% # no significance across seep at > 5 (10 points)
...
write_csv(myspecies, here("Data", "Species_Abundances_wide_subset.csv"))

# reduce species to all > 10% cover
traits <- traits %>%
  filter(Taxa %in% colnames(myspecies))


# write csv for all distinct FE combinations
# for the reduced species subset
fes_traits.sgd <- traits %>%
  select(FE = FE, Taxon_Group:FM) %>%
  distinct()

write_csv(fes_traits.sgd, here("Data", "Distinct_FE_subset.csv"))
...
write_csv(species_entities, here("Data", "Species_FE_subset.csv"))
...
qfs <- quality_funct_space(mat_funct = entity, # distinct functional trait entities (NAs not allowed. Traits can be different types: numeric, ordinal, nominal)
                           ...
                           nbdim = 7, # default = 7, max number of dimensions
...
write.csv(fd.coord.sgd, here("Data","FE_4D_coord_dmb_subset.csv"))
...
# OR Set categories of CowTagIDs (will sort later based on biogeochemistry or nutrients)
sgd.sp <- myspecies %>%
...
# option FA does not allow less than 5 entities
# remove and hand calculate SPR, %SPR, FER, %FER, and Vol(?)
# V1, 6, 7, 9, 10, 12, 14, 17, 18, 20, Seep
#richCowTags <- c("V2", "V3", "V4", "V5", "V8", "V11", "V15", "V16", "V19")

# CowTagIDs as rownames
# sgd.sp <- sgd.sp %>%
#   filter(CowTagID %in% richCowTags)
...
# avoid volume calculations for now and focus on SPR and FER

  c(length(species.sgd), length(species.sgd)/ncol(sgd.sp)*100, # species richness and relative abundance
    dim(m.sgd)[1], dim(m.sgd)[1]/dim(fd.coord.sgd)[1]*100) #, # FE richness and relative abundance
    #ch.sgd$vol/chg.sgd$vol*100) # relative volume
...
write_csv(Fric, here("Data", "Sp_FE_Vol_subset.csv"))

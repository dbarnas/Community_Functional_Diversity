---
title: "Community vs SGD Regression (with V2 removed"
...
water <- read_csv(here("Data","Biogeochem","Nutrient_Processed_CV_seasons.csv")) %>% filter(CowTagID != "V2")
...
Full_data <- div %>%
  ...
  full_join(meta) %>%
  full_join(sub) %>%
  drop_na() %>%  # remove incomplete datasets (for now, just varari w/o V13 and V2)
  filter(CowTagID != "V2")
...
spPA <- taxa %>%  
  ...
  pivot_longer(names_to = "Taxa", values_to = "presence", cols = 2:56) %>%  #long form to turn NAs into 0's for absence (lost 6 species from site total by removing V2)
...
genPA <- taxa %>% 
  ...
  pivot_longer(names_to = "Genus", values_to = "presence", cols = 2:31) %>%  #long form to turn NAs into 0's for absence (lost 1 genus by removing V2?)
  mutate(presence = if_else(is.na(presence), 0, 1)) # presence-absence of species per CowTagID
```
...
CTlevels <- c('V1','V3','V4','V5','V6','V7','V8','V9','V10','V11','V12','V14','V15','V16','V17','V18','V19','V20')
...
## Richness and Diversity ~ Rugosity
```{r}
## plot
groupplot(mydata = longspecies, x = meanRugosity, y = SpeciesVal, fw = Metric) +
  geom_text_repel(aes(label = CowTagID))

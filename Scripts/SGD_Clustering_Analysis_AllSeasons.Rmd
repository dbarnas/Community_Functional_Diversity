...
turbAug<-read_csv(here("Data","Biogeochem","August2021","Turb_NC.csv"))

MarchChemData <- read_csv(curl('https://raw.githubusercontent.com/njsilbiger/MooreaSGD_site-selection/main/Data//March2022/CarbonateChemistry/pHProbe_Data_calculated_POcorrect.csv'))

turbMarch<-read_csv(here("Data","Biogeochem","March2022","Turb_NC.csv"))
...
## combine turbinaria data
turb <- turbAug %>% 
  full_join(turbMarch)


## Isolate gps coordinates to join with March data
gps <- AugChemData %>% 
...
AugChemData <- AugChemData %>% 
  ...
  mutate(Season = "Dry") %>%  # add seasonal description
...
## Filter out unnecessary data not related to my sample locations
removeSiteMarch <- MarchChemData %>% 
...
MarchChemData <- MarchChemData %>% 
  anti_join(removeSiteMarch) %>% 
  mutate(Season = "Wet") %>%  # add seasonal description
  select(-c(TempInSitu, 
            Notes, 
            SamplingTime)) %>% 
  # add Location
...
## Join dataframes
AllChemData <- full_join(AugChemData, MarchChemData)
...
Summarise: Range of parameters across sample times
```{r}
...
Full_data <- AllChemData %>%
  ...
  ungroup() %>% 
  left_join(turb, by = c('CowTagID', 'Season'))  # join with turb
...
Full_data %>% 
  select(Location, Season, CowTagID, Silicate_umolL, del15N, N_percent) %>% 
...
Full_data %>% 
  ...
  labs(caption = paste("Varari R-squared:",round(Vsum, 2), "\n",
                       "Cabral R-squared:", round(Csum, 2)),
       x = "Range Silicate (umolL)",
       y = "Range of delta N15") +
  facet_wrap(~Location, scales = "free")+
  theme(strip.background = element_rect(fill = "white"))
```

# End of modified script. Do not run past here.
...
ggsave(here("Output","SGDZones","Varari",paste0("DiscPower_March_Varari.png")),Vdes, height = 10, width = 10)
ggsave(here("Output","SGDZones","Cabral",paste0("DiscPower_March_Cabral.png")),Cdes, height = 10, width = 10)

...
ReducedChemData <- AllChemData %>% 
  select(-c(Temperature)) %>%  # maybe not relevant for clustering
  unite(Date,Tide,Day_Night, col = date_tide_day, sep = "_", remove = F) %>% 
  filter(date_tide_day != '2021-08-06_Low_Day') %>%  # this was a very high 'low tide' so only use 2022-08-08 samples
  # only keep Plate and Seep data
  filter(Plate_Seep == "Plate" | Plate_Seep == "Seep") %>%  # ignores springs and well
  select(-c(Date,Time,DateTime, date_tide_day, Bottom_Plate_ID, Jamie_Plate_ID)) # remove unnecessary columns
  
  # some mismatched information; need to remove conflicting columns and rejoin
tempA <- ReducedChemData %>% 
  filter(is.na(Salinity)) %>% 
  select(-c(Salinity,TA,pH)) # isolate data where salinity, ta, and pH are NA and remove empty cols
tempB <- ReducedChemData %>% 
  filter(is.na(Phosphate_umolL)) %>% 
  select(-c(12:22)) # isolate data where phosphate:lignin_like are NA and remove empty cols
temp <- tempA %>% 
  full_join(tempB) # join by cowtagID to combine previously mismatched row information

ReducedChemData <- ReducedChemData %>% 
  filter(!is.na(Salinity)) %>% 
  filter(!is.na(Phosphate_umolL)) %>% 
  full_join(temp) %>% # rejoin all rows together
  group_by(Location, CowTagID) %>%
  summarise_if(.vars = 9:22, .predicate = is.numeric, .funs = mean, na.rm=T) # get mean across sampling periods by site and plate
...
x_V <- V_data[,5:20] # all biogeochemical and turb nutrient data
x_C <- C_data[,5:20]
...
res_without_V <- VarSelCluster(x_V, # matrix/data frame (only the numerical data for now) 
                             gvals = 1:16, # defines number of components (clusters) to consider
...
Vdes<-plot(x = res_without_V, y = 1:16)
Cdes<-plot(x = res_without_C, y = 1:16)

#ggsave(here("Output","DiscriminativePower_Varari_meanVals.png"),Vdes, height = 10, width = 10)
#ggsave(here("Output","DiscriminativePower_Cabral_meanVals.png"),Cdes, height = 10, width = 10)
Vdes
Cdes
```
...
#ggsave(here("Output","Varari_Cluster_Map_meanVals.png"),cluster_varari_map,height = 10, width = 10)
...
#ggsave(here("Output","Cabral_Cluster_Map_meanVals.png"),cluster_cabral_map,height = 10, width = 10)
...
#ggsave(here("Output","Boxplots_Varari_meanVals.png"), Vbox, height = 10, width = 8)
#ggsave(here("Output","Boxplots_Cabral_meanVals.png"), Cbox, height = 10, width = 8)

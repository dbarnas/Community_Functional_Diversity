...
turb1<-read_csv(here("Data","Biogeochem","Turb_NC.csv"))
```
...
remove <- AllChemData %>% filter(CowTagID=="V2", Tide ==" Low",Day_Night=="Day", Date == ymd("2021-08-08"))

remove2 <- AllChemData %>% filter(CowTagID== "C4", Tide =="Low", Day_Night=="Night", Date == ymd("2021-08-09"))

## Filter out redundant low tide day sample, first 'low tide' was super high
remove3 <- AllChemData %>%  filter(Tide == "Low", Day_Night == "Day", Date == ymd("2021-08-06"))
...
Summarise
```{r}
## Set summarise parameter
#sumParam <- "Median_Max_MinSalinity" # used to label saved files
sumParam <- "Median" # used to label saved files
...
Full_data <- ReducedChemData %>%
  ...
  summarise_at(vars(Salinity:Tyrosine_Like), .funs = median, na.rm=T) %>%  # get mean across sampling periods by site and plate
  ungroup() %>%
  full_join(turb, by = 'CowTagID') # join with turb


# ## Separate summaries
# medData <- ReducedChemData %>% 
#   group_by(Location, CowTagID, lat, lon) %>%
#   summarise_at(vars(TA:pH, Tryptophan_Like:Tyrosine_Like), .funs = median, na.rm=T) %>%
#   ungroup()
# maxData <- ReducedChemData %>%
#   group_by(Location, CowTagID, lat, lon) %>%
#   summarise_at(vars(Phosphate_umolL:Ammonia_umolL, HIX, VisibleHumidic_Like), .funs = max, na.rm=T) %>%
#   ungroup()
# minData <- ReducedChemData %>%
#   group_by(Location, CowTagID, lat, lon) %>%
#   summarise_at(vars(Salinity, M_C), .funs = min, na.rm=T) %>%
#   ungroup()
# 
# Full_data <- minData %>% # join all summaries together
#   full_join(medData) %>%
#   full_join(maxData) %>%
#   full_join(turb, by = 'CowTagID') # join with turb
...
## Check for relationships to silicate
```{r}
## first check linear relationships with silicate
ReducedChemData %>% 
  full_join(turb, by = 'CowTagID') %>% 
  pivot_longer(cols = c(Salinity:Phosphate_umolL, NN_umolL:N_percent), names_to = "Parameters", values_to = "Values") %>% 
...
Full_data %>% 
  pivot_longer(cols = c(Salinity:Phosphate_umolL, NN_umolL:N_percent), names_to = "Parameters", values_to = "Values") %>% 
  filter(Silicate_umolL < 15) %>% # remove seep data for now and C2
  ggplot(aes(x = Silicate_umolL, 
             y = Values,
             color = Location)) +
  geom_point() +
...
# plot histogram  
 p <- Full_data %>%
   ...
   geom_histogram() +
   labs(x = numbNames[i],
        title = paste0(sumParam," ",numbNames[i],": ",signif),
...
pdf(here("Output","SGDZones","histograms",paste0(sumParam,"_Histograms.pdf")), onefile = TRUE)
...
ggsave(here("Output","SGDZones","Varari",paste0("DiscriminativePower_Varari_",sumParam,".png")),Vdes, height = 10, width = 10)
ggsave(here("Output","SGDZones","Cabral",paste0("DiscriminativePower_Cabral_",sumParam,".png")),Cdes, height = 10, width = 10)
...
ggsave(here("Output","SGDZones","Moorea_Map.png"),MooreaMapPlot,height = 10, width = 10)
```

```{r}
# Varari
VmapSites <- ggmap(VarariBaseMap) +
  geom_point(data = ReducedChemData,
             aes(x = lon, y = lat),
             color = "white",
             size = 2) + 
  labs(x = "Longitude", y = "Latitude",  #label x and y axes
        title = "Varari Sample Locations") +
  geom_label(data = AllChemData,
             aes(x = lon, y = lat,
                 label = CowTagID),
             size = 2, alpha = 0.2) #,
#             color = "white")
...
## create palette
V_cluster_palette <- c("#fad02c", "white", "brown3") #1 is dark yellow #2 is baby blue #3 is red

  ### Map Varari clusters
VSeepPt <- V_Cluster_data %>% filter(CowTagID == "VSEEP") # isolate Varari seep
...
ggsave(here("Output","SGDZones","Varari",paste0("Varari_Cluster_Map_TransScale_",sumParam,".png")),cluster_varari_map,height = 10, width = 10)
...
ggsave(here("Output","SGDZones","Cabral",paste0("Cabral_Cluster_Map_TransScale_",sumParam,".png")),cluster_cabral_map,height = 10, width = 10)
...
ggsave(plot = ClusterMaps, filename = here("Output","SGDZones","Cluster_Maps.png"), width = 14, height = 10)
```
...
ggsave(here("Output","SGDZones","Varari",paste0("Boxplots_Varari_TransScale_",sumParam,".png")), Vbox, height = 20, width = 8)

ggsave(here("Output","SGDZones","Cabral",paste0("Boxplots_Cabral_TransScale_",sumParam,".png")), Cbox, height = 20, width = 8)
...
ggsave(plot = VarariPCA, filename = here("Output","SGDZones","Varari","VarariPCA.pdf"), width = 14, height = 6)
ggsave(plot = CabralPCA, filename = here("Output","SGDZones","Cabral","CabralPCA.pdf"), width = 14, height = 6)
...
Turb_cluster_data <- Scale_data %>% 
  left_join(to_join_turb, by = 'CowTagID') %>% 
...
# ztrue_V_turb <- V_data_turb[,'N_percent'] # compare clustering with %N in turb
# ztrue_C_turb <- C_data_turb[,'N_percent']
...
ggsave(here(paste0("Output","DiscriminativePower_Turb_Varari_",sumParam,".png")),Vdes_turb, height = 10, width = 10)
ggsave(here(paste0("Output","DiscriminativePower_Turb_Cabral_",sumParam,".png")),Cdes_turb, height = 10, width = 10)
...
plot(res_C_turb, y="del15N", type="cdf")
plot(res_C_turb, y="C_N", type="cdf")
...
cluster_varari_map_turb <- ggmap(VarariBaseMap) +
  ...
  labs(x = "Longitude", y = "Latitude",  #label x and y axes
       ...
       subtitle = sumParam,
       color = "Varari Clusters")

ggsave(here("Output","SGDZones","Varari",paste0("Varari_Turb_Cluster_Map_TransScale_",sumParam,".png")),cluster_varari_map_turb,height = 10, width = 10)
...
ggsave(here("Output","SGDZones","Cabral",paste0("Cabral_Turb_Cluster_Map_TransScale_",sumParam,".png")),cluster_cabral_map_turb,height = 10, width = 10)
...
Vbox_turb<-ggplot(V_cluster_long_turb, 
              mapping = aes(x = reorder_within(Nutrient_parameters,desc(Nutrient_values), V_cluster_turb), # tidytext::reorder_within allows reordering while using facet_wrap if scales = 'free_x'
                            y= Nutrient_values, 
                            color = V_cluster_turb)) +
  geom_boxplot()+
...
ggsave(here("Output","SGDZones","Varari",paste0("Boxplots_Turb_Varari_TransScale_",sumParam,".png")), Vbox_turb, height = 20, width = 8)

ggsave(here("Output","SGDZones","Cabral",paste0("Boxplots_Turb_Cabral_TransScale_",sumParam,".png")), Cbox_turb, height = 20, width = 8)

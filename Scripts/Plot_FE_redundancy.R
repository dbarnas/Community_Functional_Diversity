...
mypalette <- pnw_palette(name = "Bay", n = 28)
...
traits <- comp %>%
  ...
  select(Taxa, Taxon_Group,
         Morph,
         Life_Span,
         #Max_size,Growth_Rate, # still have NAs
         Symb,
         Calc,
         ER,
         FM) # use reduced names trait groups
fe_traits <- traits %>%
  unite(col = "FE", Morph:FM, sep = ",", remove = T) %>%
...
FE_nmds_data <- FER_pres_abs %>%
  ...
  mutate_at(vars(4:ncol(.)), .funs = ~if_else(is.na(.), 0, .))
...
myshortpal <- pnw_palette(name = "Bay", n = 12)
...
FE_Trait_Fxn <- function(mytrait){
  ...
  myplot <- fer_trait %>%
    ...
    theme_bw() +
    theme(panel.grid = element_blank()) +
    scale_fill_manual(values = myshortpal)  +
    labs(y = "FER", x = "Survey Location")
...
#   arrange(CowTagID) %>%
#   pivot_longer(cols = 4:31, names_to = "FE", values_to = "Counts") %>%

...
ct <- Full_data %>% 
  select(CowTagID)
rNbSp <- residuals(lm(data = Full_data, NbSp ~ meanRugosity)) %>% cbind(ct) %>% distinct()
rNbSpP <- residuals(lm(data = Full_data, NbSpP ~ meanRugosity)) %>% cbind(ct) %>% distinct()
rNbFEs <- residuals(lm(data = Full_data, NbFEs ~ meanRugosity)) %>% cbind(ct) %>% distinct()
rNbFEsP <- residuals(lm(data = Full_data, NbFEsP ~ meanRugosity)) %>% cbind(ct) %>% distinct()
rVol8D <- residuals(lm(data = Full_data, Vol8D ~ meanRugosity)) %>% cbind(ct) %>% distinct()
#deal with V4 repetition
rNbSp<-rNbSp[3:nrow(rNbSp),]
rNbSpP<-rNbSpP[3:nrow(rNbSpP),]
rNbFEs<-rNbFEs[3:nrow(rNbFEs),]
rNbFEsP<-rNbFEsP[3:nrow(rNbFEsP),]
rVol8D<-rVol8D[3:nrow(rVol8D),]
#rejoin to full
Full_data <- Full_data %>% 
  left_join(rNbSp) %>% select(-NbSp) %>% rename(NbSp = '.') %>% 
  left_join(rNbSpP) %>% select(-NbSpP) %>% rename(NbSpP = '.')%>% 
  left_join(rNbFEs) %>% select(-NbFEs) %>% rename(NbFEs = '.')%>% 
  left_join(rNbFEsP) %>% select(-NbFEsP) %>% rename(NbFEsP = '.')%>% 
  left_join(rVol8D) %>% select(-Vol8D) %>% rename(Vol8D = '.') %>% 
  relocate(NbSp:Vol8D, .after = CowTagID) %>% 
  distinct()
...
longcv <- Full_data %>% 
  ...
  select(CowTagID:dist_to_seep_m, Phosphate_umolL, NN_umolL, Ammonia_umolL, VisibleHumidic_Like, Salinity, M_C)
longcv %>% 

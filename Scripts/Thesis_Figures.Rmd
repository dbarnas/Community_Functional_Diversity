...
modelTableData <- modelTable %>%
  ...
  mutate(Y = if_else(Y == "resSpp", "% SR (res)",
         ...
         Parameter = if_else(Parameter == "meanRugosity", "Rugosity", 
                     if_else(Parameter == "Temperature", "Temperature", Parameter)),
         Parameter = factor(Parameter, levels = c("Rugosity", "Phosphate", "Nitrate+Nitrite", "pH", "Salinity", "Silicate", "Temperature"))) %>%
...
```
```{r, echo = F, eval = F}
compareAllAICc <- modelTableData %>% 
  mutate(Div = if_else(Y== "% SR", "SR", if_else(Y == "resSpp", "SR",
               if_else(Y== "% FER", "FER", if_else(Y== "resFEp", "FER",
               if_else(Y== "% FEV", "FEV", if_else(Y== "resVol", "FEV", ""))))))) %>% 
  group_by(Div) %>% 
  select(-delAICc, -minAICc, -deltaAICc) %>% #calculate again with div groups
  mutate(minAICc = min(AICc),
         deltaAICc = AICc - minAICc)
...
### Table/Figure X: SGD along the reef 
...
cvreef <- tabData %>% 
  ...
  labs(x = "Coefficient of Variation", color = expression("Phosphate ("*mu*"mol/L)")) +
  theme_classic() +
  theme(strip.background = element_rect(fill = "white"),
        axis.title.y = element_blank(),
...
#############################
modelData <- resFric %>% select(NbSpP, NbFEsP, Vol8D, resSpp, resFEp, resVol, Phosphate_umolL)

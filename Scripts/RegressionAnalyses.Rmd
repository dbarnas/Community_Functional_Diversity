...
groupplot(mydata = longsub, x = Silicate_umolL, y = subVal, fw = Substrate) +
  labs(x = "Silicate (umolL)", y = "% Cover of Substrate Type")+
...
# %N or silicate and substrate type: all insignificant
summary(lm(data = Full_data, Sand ~ Silicate_umolL))
summary(lm(data = Full_data, Rubble ~ Silicate_umolL))
summary(lm(data = Full_data, LiveCoral ~ Silicate_umolL))
summary(lm(data = Full_data, DeadCoral ~ Silicate_umolL))
```
...
Distance_pval %>% 
  ...
  geom_col(fill = if_else(Distance_pval$pvalue < 0.05, "red", "darkgrey")) +
...
# values which are not correlated with distance to seep
...
# ggplot
stackplot(mydata = longsub %>% filter(CowTagID != "V13"), x = CowTagID, y = subVal, colfill = Substrate) +
  labs(subtitle = "Distance to seep (m)") +
  theme(axis.text.x = element_text(angle = 90))+
...
spPA <- comp %>% 
  ...
  pivot_longer(names_to = "Taxa", values_to = "SpeciesCounts", cols = Turf:'Caulerpa racemosa') %>%  #long form to turn NAs into 0's for absence
...
pca_fulldata <- Full_data %>% 
  ungroup() %>% # to remove location and cowtag
...
### loadings and points on single plot. not a great visual
...
#         panel.grid.minor = element_blank()) +
#   labs(title = "Varari Loadings") +
#   ### Now add CowTagID points
#   geom_point(data = pca_fulldata_all %>% select(CowTagID, PC1, PC2),
#              aes(x = PC1, 
#                  y = PC2,
#                  label = CowTagID),
#              shape = 21,
...
#              fill = "white",
#              size = 3) +
#   geom_text_repel(data = pca_fulldata_all %>% select(CowTagID, PC1, PC2),
#                   aes(x = PC1,
#                       y = PC2,
#                       label = CowTagID))
#   
...
#(plot = VarariPCA, filename = here("Output","VarariPCA_meta.png"), width = 14, height = 6)
VarariPCA
```
...
pca_fulldata_all %>% 
  groupplot(x = PC1, y = dist_to_seep_m, fw = NA) +
  scale_color_manual(values = mypalette)
...
summary(lm(data = pca_fulldata_all, PC1 ~ dist_to_seep_m)) # p = 0.01
...
pca_fulldata2 <- Full_data %>% 
  ungroup() %>% # to remove location and cowtagid
  filter(CowTagID != "VSEEP", CowTagID != "V13") %>% 
  select(Salinity:N_percent,dist_to_seep_m:adj_CT_depth_cm) # avoid rugosity
...
summary(lm(data = pca_fulldata_all2, PC1 ~ dist_to_seep_m)) # insignificant for all 
...
speciesPerm <- comp %>%
  ...
  mutate_at(vars('Turf':'Caulerpa racemosa'), .funs = ~if_else(is.na(.)==TRUE, 0, .)) # put 0 %cover for all NA values
...
speciesMDS1b <- c(speciesOrd$points[,1]) # MDS1 for CowTagID
speciesMDS2b <- c(speciesOrd$points[,2]) # MDS2 for CowTagID
...
```{r}
taxonPalette <- tibble('Taxon_Group' = as.character(c("Coral", "Turf", "Chlorophyta", "Rhodophyta","Phaeophyta", "CCA", "Demospongiae", "Corallimorph", "Anemone", "Cyanobacteria")),
                       'PalColor' = as.character(c("black", "#00496F", "#086B8B","#086B8B","#086B8B", "#2E9093", "#EDC132", "#ED950A", "#DD4124", "#b99095")))
...
permanovamodel2<-adonis2(speciesPermFull[,-c(1)]~CowTagID, speciesPermFull, permutations = 999, 
                       method="bray") # should change out cowtagid with some grouping name
permanovamodel2

#If we are to trust the results of the permanova, then we have to assume that the dispersion among
#data is the same in each group. We can test with assumption with a PermDisp test:
disper<-vegdist(speciesPermFull[,-c(1)])
betadisper(disper, speciesPermFull$CowTagID)
#Look at the Average distance to median...these numbers should be reasonably similar
#A rule of thumb is that one number should not be twice as high as any other

pairwise.adonis(speciesPermFull[-1], speciesPermFull$CowTagID, perm=999)
...
Richness_pval %>% 
  ggplot(aes(x = Parameter, y = pvalue)) +
  geom_col(fill = if_else(Richness_pval$pvalue < 0.05, "red", "darkgrey")) +
  geom_hline(yintercept = 0.05) +
  theme_bw() +
...
resr2val <- summary(lm(data = res_data, res ~ dist_to_seep_m))$'r.squared'

plot_residuals_dist2seep <- res_data %>% 
  ...
  labs(y = "Residuals (Species Richness ~ Mean Rugosity)",
       ...
       caption = paste("p-value:",round(respval,3),'\n',
                       "r-squared:", round(resr2val,3)))
plot_residuals_dist2seep

ggsave(here("Output", "PaperFigures", "Residuals_Distance.png"),plot_residuals_dist2seep, width = 6, height = 5)
